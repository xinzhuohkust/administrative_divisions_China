...
while (i < length(province_sites)) {
  i = i + 1
  str_c("http://www.stats.gov.cn/tjsj/tjbz/tjyqhdmhcxhfdm/2015/", province_sites[i]) -> sites[i]
...
while (i < length(city_sites)) {
  i = i + 1
  str_c("http://www.stats.gov.cn/tjsj/tjbz/tjyqhdmhcxhfdm/2015/", str_extract(city_sites[i], "^\\d{2}"), "/", city_sites[i]) -> sites[i]
  cat("scraping url number", i, "\n")
...
# 区 ---------------------------------------------------------------------------
save.image(file = "code2015.Rdata")
webs <- list()
sites <- c()
i = 0
while (i < length(district_sites)) {
  i = i + 1
  str_c("http://www.stats.gov.cn/tjsj/tjbz/tjyqhdmhcxhfdm/2015/", str_extract(district_sites[i], "^\\d{2}"), "/", str_sub(district_sites[i], 3, -8), "/", district_sites[i]) -> sites[i]
  cat("scraping url number", i, "\n")
...
while (i < length(town_sites)) {
  i = i + 1
  str_c("http://www.stats.gov.cn/tjsj/tjbz/tjyqhdmhcxhfdm/2015/", str_extract(town_sites[i], "^\\d{2}"), "/", str_sub(town_sites[i], 3, -11), "/", str_sub(town_sites[i], 5, -9), "/", town_sites[i]) -> sites[i]
  cat("scraping url number", i, "\n")
...
communities %>% 
  ...
  left_join_dt(set_names(towns, c("镇级行政单位代码", "镇级行政单位"))) -> code_2015

save(code_2015, file = "code_2015.Rdata")
